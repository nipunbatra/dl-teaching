...
- deck source `lecture10/L10-segmentation.typ`, SHA-256 `c95df3dd7c23c1437f7ee6b9baf8415f0655027ab71035501b18aff12f39f720`;
- `notebooks/L10/01_dice_vs_ce_toy_masks.ipynb`, SHA-256 `5847c567b77dce5e70f20636ccb6d86f9067a10d0ff34a248568cad25145b2b3`.

The deck carries one exact $4\times4$ case through truth, probabilities, inference, semantic metrics, and the need for instance identity. If either artifact changes, recheck every entry of $G$, $p$, and $P$ before teaching.

## The student contract

By the end of the core route, a student should be able to:

1. distinguish semantic, instance, and panoptic output structures;
2. explain why a segmentation head preserves spatial axes and shares a classifier across locations;
3. keep the `TRAIN`, `INFER`, and `EVAL` clocks separate;
4. derive per-pixel cross-entropy and Soft Dice on the held probability field;
5. explain why an encoder needs context, a decoder needs localization, and U-Net skips preserve aligned detail;
6. threshold the held probabilities, count TP/FP/FN/TN, and compute accuracy, IoU, and Dice;
7. state a complete semantic mIoU protocol, including classes, void pixels, absent classes, and reduction;
8. explain the two stages of Mask R-CNN, the purpose of RoIAlign, and the original class-specific mask loss;
9. distinguish box AP from mask AP; and
10. diagnose a segmentation failure with a symptom → suspect → smallest-test argument.

The lecture spine is:

> A dense model trains probabilities against pixel truth, turns probabilities into a structured output at inference, and evaluates that frozen output under a declared mask protocol. Semantic evidence answers area; instance identity answers count.
...
| 4–9 | CORE | route, three clocks, held truth, task contracts | name what each clock may see | one semantic union can encode area but not two identities |
...
| 29–40 | CORE | threshold commitment, counts, accuracy, IoU, Dice, Soft Dice, mIoU | commit TP/FP/FN/TN before reveal | `5/3/1/7`; `.750/.556/.714`; Soft Dice `.727` |
...
## The held orchard case

Use this case continuously. Do not swap in a different scene, a second mask, or an ad hoc probability map midway through the lecture.
...
The semantic target withholds IDs: it contains one six-pixel union. That union supports total canopy area, while the identity map supports counting two touching trees.
...
4. Show semantic truth next to the latent two-ID map.
5. Compare semantic, instance, and panoptic contracts.
6. Promise that $G$, $p$, threshold, and $P$ will not change.
...
Commit the counts, then derive every hard metric from the same four values. Return briefly to `TRAIN` for Soft Dice, then move back to `EVAL` for mIoU. This clock switch is intentional; announce it.
...
Use exactly one notebook. It is deterministic, uses only NumPy and Matplotlib, and has no downloads or training.
...
Pinned notebook SHA-256: `5847c567b77dce5e70f20636ccb6d86f9067a10d0ff34a248568cad25145b2b3`.
...
The mask plot must remain the exact held $G/P$ pair. White digits mark foreground cells for contrast.
...
- There is no train/validation/test split because this notebook fits no parameters and selects no policy. A trained experiment must select threshold and reduction rules on validation data and freeze them before test evaluation.
...
Teach minutes 0–51 of the main route, skip directly over `SHOULD` and `OPTIONAL`, then spend four minutes on the orchard revisit and L12 handoff. Never cut the clock distinction, fixed $G/p/P$, U-Net convention, hard metrics, Mask R-CNN loss boundary, mask AP distinction, or revisit.
...
| 4–7 | three clocks and fixed $G\to p\to P$ promise |
...
2. orchard vote;
3. fixed $G,p,P$ and the three clocks;
